...
def text_to_speech_get(
    ...
    try:
        ...
        audio = tts_model.generate_audio(model_state, text)
        audio_np = audio.cpu().numpy()
        # Save to centralized tts_output folder in root directory
        # __file__ is in tts/pocket-tts/pocket_tts/main.py
        # Need to go up 4 levels to reach Gem-System root
        root_dir = PathLib(__file__).parent.parent.parent.parent
        output_filepath = root_dir / "tts_output" / "server_output.wav"
        scipy.io.wavfile.write(str(output_filepath), tts_model.sample_rate, audio_np)
