...
# --- OSC Configuration ---
# IP address of the receiving application (e.g., VRChat, Resolume, TouchDesigner)
RECEIVER_IP = "127.0.0.1"  # Use '127.0.0.1' for localhost (same machine)
# Port the receiving application is listening on
RECEIVER_PORT = 9001       # Default VRChat OSC port, change if needed
# OSC Address Pattern to send the message to
OSC_ADDRESS = "/chatbox/input" # VRChat uses this, change if needed for other apps
...
@app.route('/')
def index():
    return "Flask OSC Backend is running. Send POST requests to /receive_chat."

# --- Run the Flask App ---
if __name__ == '__main__':
    # Listen on 0.0.0.0 to be accessible from the network (and localhost)
    # The default port 13000 matches the frontend JavaScript default
    flask_port = 13000
    print(f"Starting Flask server on http://0.0.0.0:{flask_port}")
    app.run(host='0.0.0.0', port=flask_port, debug=True) # debug=True helps during development
...
if __name__ == "__main__":
    ...
    group.add_argument("--web", action="store_true", help="Run in POST server mode (Flask).")
    group.add_argument("--interactive", action="store_true", help="Run in interactive command-line mode.")
    args = parser.parse_args()

    if args.web:
        app.run(debug=False, port=13000)  # Run Flask in POST mode
